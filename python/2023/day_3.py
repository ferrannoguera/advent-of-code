...
def _parse_schematic(_first_pos, _data, _found_num, _gear_nums):
    ...
    if return_value:
        return int(_found_num), _gear_nums
    return 0, _gear_nums
...
if __name__ == '__main__':
    ...
    for row in range(parsed_input_data.shape[0]):
        if found_num:
            valid_num, gear_nums = _parse_schematic(first_pos, parsed_input_data, found_num, gear_nums)
            sum_valid_numbers += valid_num
        found_num = None
        first_pos = None
